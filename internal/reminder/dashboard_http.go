...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,rgba(61,132,115,.28),transparent 30%),var(--bg);color:var(--text);font-family:var(--font)}
    ...
    .device-list,.field-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px}.device{display:flex;gap:12px;align-items:flex-start;padding:14px;border-radius:18px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08)}
...
<body>
    ...
    <section class="panel hero">
      ...
      <div class="grid">
        ...
        <section class="panel s12">
          ...
          <div class="field-grid" style="grid-template-columns:repeat(12,minmax(0,1fr))">
            ...
            <div style="grid-column:span 6"><label for="tts-voice">Voice / 模型</label><select id="tts-voice"></select></div>
            <div style="grid-column:span 12" id="tts-style-wrap"><label for="tts-style-prompt">Gemini Style Prompt</label><input id="tts-style-prompt" type="text" placeholder="例如：請用自然的台灣中文口吻，清楚且溫和地說"></div>
            <div style="grid-column:span 12" id="tts-cloud-wrap" class="hidden">
              <div class="field-grid" style="grid-template-columns:repeat(12,minmax(0,1fr))">
                <div style="grid-column:span 4"><label for="tts-input-mode">輸入模式</label><select id="tts-input-mode"><option value="text">Text</option><option value="ssml">SSML</option></select></div>
                <div style="grid-column:span 4"><label for="tts-speaking-rate">語速</label><input id="tts-speaking-rate" type="number" step="0.05" min="0.25" max="4" placeholder="1.0"></div>
                <div style="grid-column:span 4"><label for="tts-pitch">音高</label><input id="tts-pitch" type="number" step="0.5" min="-20" max="20" placeholder="0"></div>
                <div style="grid-column:span 6"><label for="tts-volume-gain-db">音量增益 dB</label><input id="tts-volume-gain-db" type="number" step="0.5" min="-96" max="16" placeholder="0"></div>
                <div style="grid-column:span 6"><label for="tts-effects-profile">Effects Profile</label><input id="tts-effects-profile" type="text" placeholder="例如：small-bluetooth-speaker-class-device"></div>
              </div>
            </div>
        ...
        <section class="panel s12">
          ...
          <div class="field-grid" style="grid-template-columns:repeat(12,minmax(0,1fr))">
            ...
            <div style="grid-column:span 6"><label for="auto-tts-voice">Voice / 模型</label><select id="auto-tts-voice"></select></div>
            <div style="grid-column:span 12" id="auto-tts-style-wrap"><label for="auto-tts-style-prompt">Gemini Style Prompt</label><input id="auto-tts-style-prompt" type="text" placeholder="例如：請用自然的台灣中文口吻，清楚且溫和地說"></div>
            <div style="grid-column:span 12" id="auto-tts-cloud-wrap" class="hidden">
              <div class="field-grid" style="grid-template-columns:repeat(12,minmax(0,1fr))">
                <div style="grid-column:span 4"><label for="auto-tts-input-mode">輸入模式</label><select id="auto-tts-input-mode"><option value="text">Text</option><option value="ssml">SSML</option></select></div>
                <div style="grid-column:span 4"><label for="auto-tts-speaking-rate">語速</label><input id="auto-tts-speaking-rate" type="number" step="0.05" min="0.25" max="4" placeholder="1.0"></div>
                <div style="grid-column:span 4"><label for="auto-tts-pitch">音高</label><input id="auto-tts-pitch" type="number" step="0.5" min="-20" max="20" placeholder="0"></div>
                <div style="grid-column:span 6"><label for="auto-tts-volume-gain-db">音量增益 dB</label><input id="auto-tts-volume-gain-db" type="number" step="0.5" min="-96" max="16" placeholder="0"></div>
                <div style="grid-column:span 6"><label for="auto-tts-effects-profile">Effects Profile</label><input id="auto-tts-effects-profile" type="text" placeholder="例如：small-bluetooth-speaker-class-device"></div>
              </div>
            </div>
  ...
  <script>
    ...
    function isGeminiEntity(entityID){return entityID==="tts.google_ai_tts"||entityID==="tts.google_generative_ai_tts"}
    function isGoogleCloudEntity(entityID){return entityID==="tts.google_cloud_tts_direct"}
    function renderVoiceOptions(selectEl,entityID,selectedVoice){selectEl.innerHTML="";const empty=document.createElement("option");empty.value="";if(isGeminiEntity(entityID)){empty.textContent="自動（Gemini 預設用 achernar）";selectEl.appendChild(empty);["achernar","leda","kore","zephyr"].forEach((name)=>{const option=document.createElement("option");option.value=name;option.textContent=name;option.selected=name===selectedVoice;selectEl.appendChild(option)})}else if(isGoogleCloudEntity(entityID)){empty.textContent="預設 voice（依 languageCode 由 Google Cloud 決定）";selectEl.appendChild(empty);(broadcastOptions.google_cloud_voices||[]).forEach((voice)=>{const option=document.createElement("option");option.value=voice.name;option.textContent=voice.name+" / "+(voice.ssml_gender||"--");option.selected=voice.name===selectedVoice;selectEl.appendChild(option)})}else{empty.textContent="此引擎不支援 voice 選項";selectEl.appendChild(empty)}}
    function syncProviderFields(prefix){const entityID=prefix==="auto"?els.autoTTSEntity.value:els.ttsEntity.value,isGemini=isGeminiEntity(entityID),isCloud=isGoogleCloudEntity(entityID),voiceSelect=prefix==="auto"?els.autoTTSVoice:els.ttsVoice,styleWrap=prefix==="auto"?els.autoTTSStyleWrap:els.ttsStyleWrap,cloudWrap=prefix==="auto"?els.autoTTSCloudWrap:els.ttsCloudWrap,languageInput=prefix==="auto"?els.autoTTSLanguage:els.ttsLanguage,selectedVoice=voiceSelect.value;renderVoiceOptions(voiceSelect,entityID,selectedVoice);styleWrap.classList.toggle("hidden",!isGemini);cloudWrap.classList.toggle("hidden",!isCloud);if(isGemini){languageInput.value=""}}
    function updateBroadcastButtonState(){const hasMessage=els.broadcastMessage.value.trim().length>0,targets=selectedTargets("broadcast-target"),entityID=els.ttsEntity.value,isGemini=isGeminiEntity(entityID),isCloud=isGoogleCloudEntity(entityID);els.broadcastButton.disabled=!(hasMessage&&targets.length>0&&entityID);if(!hasMessage)els.broadcastSummary.textContent="請先輸入測試播報內容。";else if(targets.length===0)els.broadcastSummary.textContent="請至少勾選一台要播報的 HomePod。";else if(!entityID)els.broadcastSummary.textContent="目前找不到可用的 TTS 引擎。";else if(isGemini)els.broadcastSummary.textContent="Gemini 會自動判斷中文，語言欄位請留空；目前聲線為 "+(els.ttsVoice.value||"achernar")+"。";else if(isCloud)els.broadcastSummary.textContent="Google Cloud TTS 會直接產生 MP3 並播放；可調語速、音高、音量與 SSML。";else els.broadcastSummary.textContent="將送到 "+targets.length+" 台裝置。"}
    function updateAutoBroadcastButtonState(){const targets=selectedTargets("auto-broadcast-target"),entityID=els.autoTTSEntity.value,isGemini=isGeminiEntity(entityID),isCloud=isGoogleCloudEntity(entityID);els.autoSaveButton.disabled=!(targets.length>0&&entityID);if(targets.length===0)els.autoSummary.textContent="請至少勾選一台正式提醒要播報的 HomePod。";else if(!entityID)els.autoSummary.textContent="目前找不到可用的 TTS 引擎。";else if(isGemini)els.autoSummary.textContent="正式通知將使用 Gemini，自動判斷中文，語言欄位請留空；聲線為 "+(els.autoTTSVoice.value||"achernar")+"。";else if(isCloud)els.autoSummary.textContent="正式通知將使用 Google Cloud TTS，並套用你設定的語速、音高、音量與 SSML 模式。";else els.autoSummary.textContent="正式通知將送到 "+targets.length+" 台裝置。"}
    ...
    function renderBroadcastOptions(o){broadcastOptions=o||{media_players:[],tts_entities:[],default_tts_entity:"",google_cloud_voices:[]};renderTTSEntitySelect(els.ttsEntity,broadcastOptions.default_tts_entity);renderDeviceCheckboxes(els.deviceList,"broadcast-target",[]);Array.from(document.querySelectorAll("input[name='broadcast-target']")).forEach((input)=>input.addEventListener("change",updateBroadcastButtonState));syncProviderFields("test");updateBroadcastButtonState()}
    function applyAutoBroadcastSettings(settings){autoBroadcastSettings=settings||{target_entity_ids:[],tts_entity_id:broadcastOptions.default_tts_entity||"",language:"",voice:"",input_mode:"text",speaking_rate:0,pitch:0,volume_gain_db:0,effects_profile_id:"",style_prompt:""};renderTTSEntitySelect(els.autoTTSEntity,autoBroadcastSettings.tts_entity_id);els.autoTTSLanguage.value=autoBroadcastSettings.language||"";els.autoTTSStylePrompt.value=autoBroadcastSettings.style_prompt||"";els.autoTTSInputMode.value=autoBroadcastSettings.input_mode||"text";els.autoTTSSpeakingRate.value=autoBroadcastSettings.speaking_rate||"";els.autoTTSPitch.value=autoBroadcastSettings.pitch||"";els.autoTTSVolumeGainDB.value=autoBroadcastSettings.volume_gain_db||"";els.autoTTSEffectsProfile.value=autoBroadcastSettings.effects_profile_id||"";renderDeviceCheckboxes(els.autoDeviceList,"auto-broadcast-target",autoBroadcastSettings.target_entity_ids||[]);Array.from(document.querySelectorAll("input[name='auto-broadcast-target']")).forEach((input)=>input.addEventListener("change",updateAutoBroadcastButtonState));syncProviderFields("auto");els.autoTTSVoice.value=autoBroadcastSettings.voice||"";renderVoiceOptions(els.autoTTSVoice,els.autoTTSEntity.value,autoBroadcastSettings.voice||"");updateAutoBroadcastButtonState()}
    ...
    async function submitBroadcast(){const payload={message:els.broadcastMessage.value.trim(),target_entity_ids:selectedTargets("broadcast-target"),tts_entity_id:els.ttsEntity.value,language:els.ttsLanguage.value.trim(),voice:els.ttsVoice.value,input_mode:els.ttsInputMode.value,speaking_rate:Number(els.ttsSpeakingRate.value||0),pitch:Number(els.ttsPitch.value||0),volume_gain_db:Number(els.ttsVolumeGainDB.value||0),effects_profile_id:els.ttsEffectsProfile.value.trim(),style_prompt:els.ttsStylePrompt.value.trim()};els.broadcastButton.disabled=true;els.broadcastResult.textContent="正在送出測試播報...";els.broadcastResult.className="message";try{const r=await fetch(broadcastTestURL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),text=await r.text();if(!r.ok)throw new Error(text||("HTTP "+r.status));els.broadcastResult.textContent="已送出測試播報到："+payload.target_entity_ids.join(" / ");els.broadcastResult.className="message success"}catch(e){els.broadcastResult.textContent="播報送出失敗："+e.message;els.broadcastResult.className="message error"}finally{updateBroadcastButtonState()}}
    async function saveAutoBroadcastSettings(){const payload={target_entity_ids:selectedTargets("auto-broadcast-target"),tts_entity_id:els.autoTTSEntity.value,language:els.autoTTSLanguage.value.trim(),voice:els.autoTTSVoice.value,input_mode:els.autoTTSInputMode.value,speaking_rate:Number(els.autoTTSSpeakingRate.value||0),pitch:Number(els.autoTTSPitch.value||0),volume_gain_db:Number(els.autoTTSVolumeGainDB.value||0),effects_profile_id:els.autoTTSEffectsProfile.value.trim(),style_prompt:els.autoTTSStylePrompt.value.trim()};els.autoSaveButton.disabled=true;els.autoResult.textContent="正在儲存正式播報設定...";els.autoResult.className="message";try{const r=await fetch(broadcastAutoURL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),text=await r.text();if(!r.ok)throw new Error(text||("HTTP "+r.status));applyAutoBroadcastSettings(JSON.parse(text));els.autoResult.textContent="正式通知播報設定已更新。";els.autoResult.className="message success"}catch(e){els.autoResult.textContent="儲存正式播報設定失敗："+e.message;els.autoResult.className="message error"}finally{updateAutoBroadcastButtonState()}}
    els.broadcastMessage.addEventListener("input",updateBroadcastButtonState);els.ttsEntity.addEventListener("change",()=>{syncProviderFields("test");updateBroadcastButtonState()});els.ttsVoice.addEventListener("change",updateBroadcastButtonState);els.ttsStylePrompt.addEventListener("input",updateBroadcastButtonState);els.broadcastButton.addEventListener("click",submitBroadcast);els.autoTTSEntity.addEventListener("change",()=>{syncProviderFields("auto");updateAutoBroadcastButtonState()});els.autoTTSVoice.addEventListener("change",updateAutoBroadcastButtonState);els.autoTTSStylePrompt.addEventListener("input",updateAutoBroadcastButtonState);els.autoSaveButton.addEventListener("click",saveAutoBroadcastSettings);els.historyDate.addEventListener("change",(e)=>loadHistoryDay(e.target.value));
